...
describe('DonationForm', () => {
  ...
  it('should call the onDonation function with the form values when submitted', () => {
    ...
    const { getByLabelText, getByText } = render(<DonationForm onDonation={onDonation} />);

    fireEvent.change(getByLabelText('Food Item:'), { target: { value: 'Apples' } });
    fireEvent.change(getByLabelText('Quantity:'), { target: { value: '5' } });
    fireEvent.change(getByLabelText('Expiration Date:'), { target: { value: '2023-04-01' } });
